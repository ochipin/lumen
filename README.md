...
| --- | --- | --- |
| `BaseColor` | `'#07143D'` | 配色の基準色。空文字は無効なので、省略するか有効な色を指定 |
...
記事のSNS共有画像は、ページの `params.image` で指定します。例えば `content/posts/intro/index.md` と同じフォルダーに `cover.png` を置いた場合は、`image: cover.png` でその添付画像を使えます。外部URLや `/images/share.png` のようなサイト内パスも指定できます。サイト共通の `params.socialImage` は常にサイト側の画像パスとして扱います。
...
`#RGB` または `#RRGGBB` を指定します。YAMLでは `#` がコメントとして扱われないよう、引用符で囲んでください。省略時は濃紺の `#07143D` です。例えば `#245CF2` なら明るい青、`#2E7D32` なら緑に変わります。
...
囲みなどのShortcodes内で `## 見出し {#example}` と指定した場合も、ページ内でそのIDが一意なら `#example` でリンクできます。既存の接頭辞付き見出しURLも引き続き使用できます。
...
開発時の回帰テストと実行環境は [tests/README.md](tests/README.md) を参照してください。通常のテーマ利用にテスト用の依存関係は不要です。
